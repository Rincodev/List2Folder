...
# --- PyInstaller fix for ytmusicapi gettext translations ---
import gettext as _gettext

_real_translation = _gettext.translation
def _safe_translation(domain, localedir=None, languages=None, fallback=False, **kwargs):
    # ytmusicapi looks for domain="base" translations; in PyInstaller onefile .mo may be missing
    if domain == "base":
        fallback = True
    return _real_translation(domain, localedir=localedir, languages=languages, fallback=fallback, **kwargs)
...
def extract_ytmusic_playlist_id(value: str) -> str:
    """
    Accepts:
      - playlist ID: PL..., OLAK5uy..., VLPL...
      - full URL: https://music.youtube.com/playlist?list=PL...
    Returns clean playlist id for ytmusicapi.get_playlist()
    """
    v = value.strip()

    # Extract from URL ?list=...
    m = re.search(r"[?&]list=([^&]+)", v)
    if m:
        v = m.group(1)

    # ytmusic sometimes prefixes playlist ids with "VL"
    if v.startswith("VL") and len(v) > 2:
        v = v[2:]

    return v


def fetch_ytmusic_playlist_tracks(playlist_id_or_url: str, browser_json_path: Path) -> list[tuple[str, str]]:
    """
    Returns list of (artist, title) from YouTube Music playlist.
    Requires ytmusicapi browser auth file (browser.json), generated by:
      ytmusicapi browser
    """
    from ytmusicapi import YTMusic
    from ytmusicapi.exceptions import YTMusicUserError

    playlist_id = extract_ytmusic_playlist_id(playlist_id_or_url)

    # Small sanity check: user passed "Cyberpunk" etc.
    if not re.match(r"^(PL|OLAK5|RD|UU|LL)[A-Za-z0-9_\-]+", playlist_id):
        raise SystemExit(
            "[ERR] For YouTube Music, --playlist must be a playlist ID (from URL `list=...`), "
            "not a name. Example: https://music.youtube.com/playlist?list=PLxxxx -> use PLxxxx"
        )

    try:
        # IMPORTANT: pass PATH STRING to browser.json (not dict, not auth=...)
        ytm = YTMusic(str(browser_json_path))
    except YTMusicUserError as e:
        msg = str(e)
        # If someone accidentally passes oauth.json or dict-style tokens, ytmusicapi asks for oauth_credentials
        if "oauth" in msg.lower() and "oauth_credentials" in msg:
            raise SystemExit(
                "[ERR] Your auth file looks like OAuth credentials, but this tool expects browser auth.\n"
                "Generate browser.json via:\n"
                "  ytmusicapi browser\n"
                "and pass it with:\n"
